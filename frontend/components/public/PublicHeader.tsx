    ...
    <header className="sticky top-0 z-40 border-b border-tfa-border-subtle bg-tfa-background/90 text-tfa-text backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-6 px-5 lg:px-8">
        <Link href="/" className="flex shrink-0 items-center gap-3" aria-label="TFA home">
          <img src={theme === "dark" ? "/tfa-logo-white.png" : "/tfa-logo-black.png"} alt="TESL Football Association" className="h-10 w-10 object-contain" />
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={toggleTheme} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`} title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`} className="flex h-10 w-10 items-center justify-center rounded-lg border border-tfa-border-subtle bg-tfa-surface text-tfa-text transition-colors hover:bg-tfa-surface-hover">
            {theme === "dark" ? <Sun size={16} /> : <Moon size={16} />}
          </button>
          <Link href="/login" style={{ backgroundColor: "var(--tfa-accent)", color: "var(--tfa-accent-foreground)" }} className="inline-flex items-center justify-center rounded-lg px-4 py-2.5 text-xs font-semibold transition-opacity hover:opacity-85">Log in / Sign up</Link>
        </div>
      </div>
    </header>
  );
